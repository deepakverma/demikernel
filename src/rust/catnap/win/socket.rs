...
impl Socket {
    ...
    pub fn start_disconnect(&self, overlapped: *mut OVERLAPPED) -> StartIoResult {
        let result: bool = unsafe { self.extensions.disconnectex.unwrap()(self.s, overlapped, 0, 0).as_bool() };

        get_overlapped_api_result(result, 0, 0)
    }
    ...
    pub fn start_accept(&self, mut accept_result: Pin<&mut AcceptState>, overlapped: *mut OVERLAPPED) -> StartIoResult {
        ...
        get_overlapped_api_result(success, 0, bytes_out).and_then(|r: Option<OverlappedResult>| {
    ...
    pub fn start_conect(&self, remote: SocketAddr, overlapped: *mut OVERLAPPED) -> StartIoResult {
        ...
        get_overlapped_api_result(success, 0, 0)
    }
    ...
    pub fn start_pop(&self, mut pop_state: Pin<&mut PopState>, overlapped: *mut OVERLAPPED) -> StartIoResult {
        ...
        get_overlapped_api_result(success && bytes_transferred > 0, 0, bytes_transferred)
    }
    ...
    pub fn finish_pop(
        ...
        result: OverlappedResult,
    ) -> Result<(usize, Option<SocketAddrV4>), Fail> {
        if let Err(err) = result.ok() {
            return Err(err);
        }

        let addr: Option<SocketAddrV4> = if pop_state.addr_len > 0 {
    ...
    pub fn start_push(
        ...
        get_overlapped_api_result(success, 0, bytes_transferred)
    }
