...
use super::error::translate_ntstatus;

//======================================================================================================================
// Types
//======================================================================================================================
/// Type alias for result of overlapped I/O initiatiation routines.
pub type StartIoResult = Result<Option<OverlappedResult>, Fail>;
...
impl OverlappedResult {
    pub fn new(completion_key: usize, result: NTSTATUS, bytes_transferred: u32) -> OverlappedResult {
        OverlappedResult {
            completion_key,
            result,
            bytes_transferred,
    ...
    fn from_completion(overlapped: &OVERLAPPED, completion_key: usize) -> Self {
        Self {
            completion_key,
...
impl IoCompletionPort {
    ...
    /// outlive this function. An example here may be a fail cancellation: a call to `ReadFile` or `WSARecv` may be in
    ...
    where
        S: 'static,
        for<'a> F1: FnOnce(Pin<&'a mut S>, *mut OVERLAPPED) -> StartIoResult,
        ...
        for<'a> F3: FnOnce(Pin<&'a mut S>, OverlappedResult) -> Result<R, Fail>,
    {
        ...
        match start(completion.get_state_ref(), overlapped) {
            ...
            Ok(Some(result)) => {
                let result: Result<R, Fail> = finish(completion.get_state_ref(), result);

                // Clear the yielder handle to abandon the overlapped event. This call won't wait on completion.
                completion.abandon();
                return result;
            },
            ...
            Ok(None) => loop {
                ...
                if completion.inner.yielder_handle.is_none() {
                    return finish(
                        ...
                        OverlappedResult::from_completion(
    ...
    where
        F1: FnOnce(*mut OVERLAPPED) -> StartIoResult,
        ...
        F3: FnOnce(OverlappedResult) -> Result<R, Fail>,
    {
        self.do_io_with(
            (),
            yielder,
            |_, overlapped: *mut OVERLAPPED| -> StartIoResult { start(overlapped) },
...
impl<S> StatefulOverlappedCompletion<S> {
    ...
    fn clear_yielder(self: &mut Pin<Box<Self>>) {
