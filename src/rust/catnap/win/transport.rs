...
impl SharedCatnapTransport {
    pub fn new(config: &Config, mut runtime: SharedDemiRuntime) -> Self {
        ...
        let mut epoll_me: Self = me.clone();
        runtime
            .insert_background_coroutine(
                "catnap::transport::epoll",
                Box::pin(async move { epoll_me.epoll().await }),
            )
    ...
    pub async fn accept(&mut self, socket: &Socket, yielder: Yielder) -> Result<(Socket, SocketAddrV4), Fail> {
        let start = |accept_result: Pin<&mut AcceptState>, overlapped: *mut OVERLAPPED| -> StartIoResult {
    ...
    pub async fn connect(&mut self, socket: &Socket, remote: SocketAddrV4, yielder: Yielder) -> Result<(), Fail> {
        unsafe {
            self.0.iocp.do_io(
                &yielder,
                |overlapped: *mut OVERLAPPED| -> StartIoResult { socket.start_conect(remote.into(), overlapped) },
    ...
    pub async fn pop(
        ...
        unsafe {
            self.0.iocp.do_io_with(
                ...
                &yielder,
                |pop_state: Pin<&mut PopState>, overlapped: *mut OVERLAPPED| -> StartIoResult {
                ...
                |pop_state: Pin<&mut PopState>,
                 ...
                 -> Result<(usize, Option<SocketAddrV4>), Fail> {
                    socket.finish_pop(pop_state, result)
                },
            )
        }
        .await
        ...
        loop {
            let result: Result<usize, Fail> = unsafe {
                self.0.iocp.do_io_with(
                    buf.clone(),
                    &yielder,
                    |buffer: Pin<&mut DemiBuffer>, overlapped: *mut OVERLAPPED| -> StartIoResult {
